...
import com.example.videoLocadora.exceptions.AtorNaoEncontradoException;
...
public class AtorService {
	...
	/*public AtorDTO buscarPorId(Long id) {
		return new AtorDTO(atorRepository.findById(id).get());
		//Ator ator = atorRepository.findById(id)
		//        .orElseThrow(() -> new AtorNaoEncontradoException("Não foi encontrado ator com o ID informado!"));
		    
		//    return new AtorDTO(ator);
	}*/
	
	public AtorDTO buscarPorId(Long id) {
	    AtorEntity ator = atorRepository.findById(id).orElseThrow(() -> new AtorNaoEncontradoException("Ator não encontrado com o ID: " + id));
